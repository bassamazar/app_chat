...
export default function ChatPage() {
  ...
  const fetchMessages = async (chat: any) => {
    try {
      ...
      const fetchedMessages = Array.isArray(res.data) ? res.data : (res.data.data || res.data.messages || []);
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      await axiosClient.post('/groups/create', { title: groupTitle, participantIds: selectedFriends });
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100%', fontFamily: 'Arial, sans-serif', backgroundColor: '#f0f2f5', direction: 'ltr' }}>
      ...
      <div style={{ width: '320px', backgroundColor: '#ffffff', borderRight: '1px solid #ddd', display: 'flex', flexDirection: 'column' }}>
        <div style={{ height: '70px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 20px', borderBottom: '1px solid #ddd', position: 'relative' }}>
          ...
          <button onClick={() => { setShowRequests(!showRequests); fetchSentRequests(); }} style={{ background: 'none', border: 'none', cursor: 'pointer', position: 'relative', color: '#555', padding: '5px' }}>
          ...
          {showRequests && (
            <div style={{ position: 'absolute', top: '65px', right: '15px', zIndex: 10, backgroundColor: 'white', padding: '15px', borderRadius: '8px', boxShadow: '0 4px 15px rgba(0,0,0,0.15)', width: '280px', maxHeight: '400px', overflowY: 'auto' }}>
              ...
              {friendRequests.length === 0 ? <p style={{ fontSize: '13px', color: '#888', margin: '10px 0' }}>No new requests.</p> : 
              ...
              {sentRequests.length === 0 ? <p style={{ fontSize: '13px', color: '#888', margin: '10px 0' }}>No pending requests.</p> : 
                sentRequests.map((req) => (
                  <div key={req.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    <span style={{ fontSize: '14px' }}>{req.username}</span>
                    <span style={{ fontSize: '11px', fontWeight: 'bold', padding: '4px 8px', borderRadius: '12px', backgroundColor: req.status === 'PENDING' ? '#fff3cd' : '#f8d7da', color: req.status === 'PENDING' ? '#856404' : '#721c24' }}>
        ...
        <div style={{ padding: '15px', borderBottom: '1px solid #ddd', display: 'flex', gap: '10px', alignItems: 'center' }}>
          <input type="text" placeholder="Search chats..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{ flex: 1, padding: '10px 15px', borderRadius: '20px', border: '1px solid #ccc', outline: 'none' }} />
          <button onClick={() => setShowUnreadOnly(!showUnreadOnly)} style={{ padding: '8px 12px', borderRadius: '20px', border: '1px solid #ccc', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px', backgroundColor: showUnreadOnly ? '#007bff' : '#e9ecef', color: showUnreadOnly ? 'white' : '#555', transition: '0.2s' }}>
            {showUnreadOnly ? 'All' : 'Unread'}
          </button>
        </div>

        <div style={{ flex: 1, overflowY: 'auto' }}>
          {filteredChats.length === 0 ? <p style={{ textAlign: 'center', color: '#888', marginTop: '20px' }}>No chats found.</p> : 
            filteredChats.map((chat) => (
              <div key={chat.id} onClick={() => handleSelectChat(chat)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px', cursor: 'pointer', borderBottom: '1px solid #f0f0f0', backgroundColor: activeChat?.id === chat.id ? '#e9ecef' : 'transparent' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ height: '70px', backgroundColor: '#ffffff', borderBottom: '1px solid #ddd', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 20px', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', position: 'relative' }}>
            ...
            {activeChat && activeChat.isGroup && (
              <button onClick={() => setShowGroupSettings(!showGroupSettings)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px', borderRadius: '50%', backgroundColor: '#e9ecef' }}>⚙️</button>
            )}
                    ...
                    return (
                      <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                          <span style={{ fontSize: '13px' }}>{p.username} {p.id === currentUser.id ? '(You)' : ''}</span>
                          {p.role === 'ADMIN' && <span style={{ fontSize: '9px', backgroundColor: '#ffd700', padding: '1px 4px', borderRadius: '4px' }}>ADMIN</span>}
                        </div>
                        <div style={{ display: 'flex', gap: '4px' }}>
                          {p.id !== currentUser.id && (
                            <button onClick={() => handleToggleBlock(p.id)} style={{ background: isBlocked ? '#28a745' : '#6c757d', color: 'white', border: 'none', borderRadius: '4px', padding: '2px 6px', fontSize: '11px', cursor: 'pointer' }}>
                              {isBlocked ? 'Unblock' : 'Block'}
                            </button>
                          )}
                          {!isFriend && p.id !== currentUser.id && (
                            <button onClick={() => handleAddFriend(p.username)} style={{ background: '#007bff', color: 'white', border: 'none', borderRadius: '4px', padding: '2px 6px', fontSize: '11px', cursor: 'pointer' }}>+ Add</button>
                          )}
                          {(activeChat.participants.find((u: any) => u.id === currentUser.id)?.role === 'ADMIN' || p.id === currentUser.id) && p.role !== 'ADMIN' && (
                            <button onClick={() => handleRemoveMemberFromGroup(p.id)} style={{ background: '#dc3545', color: 'white', border: 'none', borderRadius: '4px', padding: '2px 6px', fontSize: '11px', cursor: 'pointer' }}>Remove</button>
                          )}
                        </div>
                      </div>
                    );
                ...
                {activeChat.participants?.find((p: any) => p.id === currentUser.id)?.role === 'ADMIN' && (
                  <>
                    ...
                    <div style={{ maxHeight: '100px', overflowY: 'auto' }}>
                      {friends.filter(f => !activeChat.participants.some((p: any) => p.id === f.id)).length === 0 ? <p style={{ fontSize: '12px', color: '#888' }}>All friends are in this group</p> : 
                        friends.filter(f => !activeChat.participants.some((p: any) => p.id === f.id)).map(friend => (
                          <div key={friend.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                            ...
                            <button onClick={() => handleAddMemberToGroup(friend.id, friend.username)} style={{ background: '#28a745', color: 'white', border: 'none', borderRadius: '4px', padding: '2px 8px', fontSize: '11px', cursor: 'pointer' }}>Add</button>
                          </div>
                        ))
                      }
                    </div>
            ...
            <button onClick={() => { setShowCreateGroup(!showCreateGroup); setShowAddFriend(false); setShowGroupSettings(false); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#555', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px', borderRadius: '50%', backgroundColor: '#e9ecef' }}>
            ...
            {showCreateGroup && (
              <div style={{ position: 'absolute', top: '50px', right: '50px', zIndex: 10, backgroundColor: 'white', padding: '15px', borderRadius: '8px', boxShadow: '0 4px 15px rgba(0,0,0,0.15)', width: '250px' }}>
                ...
                <input type="text" placeholder="Group Title..." value={groupTitle} onChange={(e) => { setGroupTitle(e.target.value); setGroupError(''); }} style={{ width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '6px', border: '1px solid #ccc', boxSizing: 'border-box', outline: 'none' }} />
            ...
            <button onClick={() => { setShowAddFriend(!showAddFriend); setShowCreateGroup(false); setShowGroupSettings(false); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#555', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px', borderRadius: '50%', backgroundColor: '#e9ecef' }}>
            ...
            {showAddFriend && (
              <div style={{ position: 'absolute', top: '50px', right: '0', zIndex: 10, backgroundColor: 'white', padding: '15px', borderRadius: '8px', boxShadow: '0 4px 15px rgba(0,0,0,0.15)', width: '250px' }}>
                <input type="text" placeholder="Type exact username..." value={newFriendName} onChange={(e) => { setNewFriendName(e.target.value); setAddError(''); }} style={{ width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '6px', border: '1px solid #ccc', boxSizing: 'border-box', outline: 'none' }} />
        ...
        <div style={{ flex: 1, padding: '20px', overflowY: 'auto', backgroundColor: '#e5ddd5' }}>
          {!activeChat ? (
            ...
            messages.map((msg, index) => {
              ...
              return (
                <div key={msg.id || index} style={{ display: 'flex', justifyContent: msg.senderId === currentUser.id ? 'flex-end' : 'flex-start', marginBottom: '10px' }}>
                  <div style={{ 
                    ...
                    {msg.type === 'IMAGE' ? (
                      <div>
                        ...
                        {contentText && <p style={{ margin: '5px 0 0 0' }}>{contentText}</p>}
                      </div>
                    ) : msg.type === 'AUDIO' ? (
                      ...
                      <div style={{ marginTop: '5px' }}>
                        <a href={msg.fileUrl} target="_blank" rel="noopener noreferrer" style={{ color: '#007bff', textDecoration: 'underline', fontWeight: 'bold' }}>
        ...
        {activeChat && (
          <div style={{ padding: '15px', backgroundColor: '#f0f0f0', display: 'flex', gap: '10px', alignItems: 'center', borderTop: '1px solid #ddd' }}>
            ...
            <button 
              ...
              style={{ padding: '8px', borderRadius: '50%', border: 'none', backgroundColor: '#e9ecef', cursor: isUploading ? 'wait' : 'pointer', fontSize: '18px' }}
              title="Attach File"
            >
              📎
            </button>
            ...
            <button 
              ...
              disabled={isUploading || (!activeChat.isGroup && blockedByMe.includes(activeChat.id))}
              style={{ 
                padding: '8px', borderRadius: '50%', border: 'none', fontSize: '18px',
            ...
            <input 
              type="text" 
              placeholder={isUploading ? "Uploading..." : (!activeChat.isGroup && blockedByMe.includes(activeChat.id) ? "You blocked this user." : "Type a message...")} 
              disabled={isUploading || (!activeChat.isGroup && blockedByMe.includes(activeChat.id))}
              value={message}
